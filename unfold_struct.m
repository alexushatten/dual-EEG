function unfold_struct(SC,varargin)
%unfold_struct Unfolds a structure.
%   unfold_struct(SC) displays the content of a variable. If SC is a structure it
...
%   unfold_struct(SC,NAME) uses NAME as the name of SC.
%   unfold_struct(SC,SHOW) If SHOW is false only the fieldnames and their sizes
%   are  shown, if SHOW is true the contents are shown also.
%   unfold_struct(SC,NAME,SHOW)
    ...
    for h=1:hmax
        ...
        for i=1:NF
            ...
            if isstruct(SC(h).(F{i}))
                unfold_struct(SC(h).(F{i}),Namei,show);
            else
                if iscell(SC(h).(F{i}))
                    siz = size(SC(h).(F{i}));
                    NC = numel(SC(h).(F{i}));
