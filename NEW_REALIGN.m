...
SubjDir = {'patient2','patient1','patient3',...
    'patient5','patient7','patient6',...
    'patient4','patient8','patient9'};
...
for s = 1:length(SubjReadyForCleanCheck)
    ...
    for n = 1:length(Prots)
        ...
        CleanedMrk = dual_clean_mrk_NEW([FPOI{n}{2},'.mrk'],[FPOI{n}{1},'.mrk']);
        warning on %#ok<WNON>
        ...
        if ~isempty(CleanedMrk) % sometimes there are not overlapping markers !
            ...
            save(fullfile(TargetDir,['icEEGi_',Subj,'_',Type,'_',Part,'.mat']),'ic_EEGialigned','ic_Hdr','NewSamplingFreq');
            save(fullfile(TargetDir,['icEEGi_',Subj,'_',Type,'_',Part,'_mrk.mat']),'ic_L_aligned_ALL','ic_T1_aligned_ALL','ic_T2_aligned_ALL');
            
            fprintf('Saving hdEEG data...\n')
            save(fullfile(TargetDir,['hdEEG_',Subj,'_',Type,'_',Part,'.mat']),'hd_EEGaligned','hd_Hdr');
            save(fullfile(TargetDir,['hdEEG_',Subj,'_',Type,'_',Part,'_mrk.mat']),'hd_L_aligned_ALL','hd_T1_aligned_ALL','hd_T2_aligned_ALL');
