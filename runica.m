...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-------------------------------------------------------------------------
% Renaud Marquis @ FBMlab, May 2019: changed all inv() calls to pinv() to
% avoid singularity issues and NaN after multiplication with sphering matrix.
%-------------------------------------------------------------------------
...
%   sphere = 2.0*inv(sqrtm(cov(data'))); % find the "sphering" matrix = spher()
  sphere = 2.0*pinv(sqrtm(cov(data'))); % find the "sphering" matrix = spher()
  % #RM@FBMlab: changed inv to pinv to avoid singularity issues and NaN
  % when multiplying sphering matrix with data
  if ~weights,
      if verbose,
...
%           weights = pinv(winv)*inv(sphere); % re-invert the component maps
          weights = pinv(winv)*pinv(sphere); % re-invert the component maps
          % #RM@FBMlab: changed inv to pinv to avoid singularity issues and NaN
          % when multiplying sphering matrix with data
      end
