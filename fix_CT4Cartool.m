function fix_CT4Cartool( ImFilePath, FixImSuffix )
% Fix CT image file by flipping / permuting dimensions of the image data
% and corresponding image header so that Cartool can read it and not mess
% it later on... In addition to fix_NifTi4Cartool.m, it also shifts image
% data to avoid negative values, convert values to integer and prevents
% values to go beyond 255 (maximum value handled by Cartool...)
%
% fix_CT4Cartool( ImFilePath, FixImSuffix )
%
%  Inputs
% --------
...
%  Outputs
% ---------
% Fixed CT file converted to Analyze format with suffix
% 'FixImSuffix'.
%
%   See also FIX_NIFTI4CARTOOL
%
%-------------------------------------------------------------------------
...
%% Shift image data such that there are no negative values:
Dnew = int16(255.*((D - min(D(:)))/(max(D(:)) - min(D(:)))));
Vnew.dt = [16 0];
