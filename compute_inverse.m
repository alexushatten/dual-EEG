function [ RISvect, RISnorm, RISsvd, OptimalReg ] = compute_inverse( EEG, ISfile, Regularization )
% compute_inverse: compute results of inverse solutions given EEG traces
% and inverse solution matrix (.is file from Cartool)
%
% [ RISvect, RISnorm, RISsvd, OptimalReg ] = compute_inverse( EEG, ISfile )
% [ RISvect, RISnorm, RISsvd, OptimalReg ] = compute_inverse( EEG, ISfile, Regularization )
%
%  Inputs
% --------
...
if strcmpi(Output,'all') || strcmpi(Output,'optimal')
    ...
    AllMax = [max(abs(ISr(:))),max(abs(EEG(:)))];
    AllMin = [min(abs(ISr(:))),min(abs(EEG(:)))];
    if ~(sqrt(min(AllMin))<realmin('single')) && ...
        ...
        EEG = single(EEG);
		ISr = single(ISr);
        Sources = nan(IShdr.numsolutionpoints,size(EEG,2),3,'single');
    else
        ...
        try
            Sources = nan(IShdr.numsolutionpoints,size(EEG,2),3);
        catch ME
    ...
    if strcmpi(Output,'all')
        ...
        try
            ...
            Sources = nan(IShdr.numsolutionpoints,size(EEG,2),size(ISr,1)+IShdr.numregularizations);
            % else
        catch ME
            FlagHuge = true;
            if strcmp(ME.identifier,'MATLAB:array:SizeLimitExceeded')
    ...
    for reg = 1:IShdr.numregularizations
        ...
        Sources(:,:,1) = squeeze(ISr(1,:,:,reg))*EEG;
        Sources(:,:,2) = squeeze(ISr(2,:,:,reg))*EEG;
        Sources(:,:,3) = squeeze(ISr(3,:,:,reg))*EEG;
...
%             if strcmpi(Output,'all')
    ...
    if strcmpi(Output,'optimal')
        ...
        % Clean memory
        clearvars -except EEG ISfile OptimalReg
        
%         % Clean up temporary files
%         rmdir(fileparts(TempFile));
        
        [RISvect,RISnorm,RISsvd] = compute_inverse(EEG,ISfile,OptimalReg);
...
%         end
    ...
    if ~(sqrt(min(AllMin))<realmin('single')) && ...
        ...
        EEG = single(EEG);
		ISr = single(ISr);
        RISvect = nan(IShdr.numsolutionpoints,size(EEG,2),3,'single');
    else
    	warning('Temporary conversion to single-precision float might lead to loss of precision in the results !')
    	warning('Keeping double-precision float... This might lead to out-of-memory errors !')
        try
            RISvect = nan(IShdr.numsolutionpoints,size(EEG,2),3);
        catch ME
            if strcmp(ME.identifier,'MATLAB:array:SizeLimitExceeded')
                warning(ME.message)
            end
    ...
    RISvect(:,:,1) = squeeze(ISr(1,:,:,Regularization))*EEG;
    RISvect(:,:,2) = squeeze(ISr(2,:,:,Regularization))*EEG;
    RISvect(:,:,3) = squeeze(ISr(3,:,:,Regularization))*EEG;
