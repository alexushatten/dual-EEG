function [Shifts,ToCheck] = spike_aligner_v2( File, MarkerOfInterest, ChannelOfInterest, FrameLength )
% spike_aligner_v2: align epochs of spikes with reference to channel with
%   maximal absolute amplitude
%
% [Shifts, ToCheck] = spike_aligner_v2( File, MarkerOfInterest, ChannelOfInterest, FrameLength )
%
%  Inputs
% --------
...
%     RefX = repmat(round(FrameLength/2)+1,FrameLength+1,1);
    ...
    [Shifts(m), Exit, ToCheck(m), Prev] = overlay_spike_on_avg_spike(EEGb(ChannelToShow,Frames)',RefY*5,labelsB(ChannelToShow),ToCheck(m),m,length(Marker_Time),File,MarkerOfInterest);
    if Prev
        if m == 1
...
function [Shift, Exit, ToCheck, Prev] = overlay_spike_on_avg_spike(EEG,AvgSpike,ChanelLabel,ToCheck,Current,Total,File,MarkerOfInterest)
