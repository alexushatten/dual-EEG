function [Shifts,ToCheck] = spike_aligner_icEEG( File, MarkerOfInterest, OldFile, OldMarkerOfInterest, ChannelOfInterest, FrameLength )
% spike_aligner_icEEG: align epochs of spikes with reference to channel with
%   maximal absolute amplitude
%
% [Shifts, ToCheck] = spike_aligner_icEEG( File, MarkerOfInterest, ChannelOfInterest, FrameLength )
%
%  Inputs
% --------
...
[~,~,labelsB,~,EEGb] = bipolar_montage(Hdr.channelnames,2,EEG);
...
if ~strcmp(File,OldFile) % in case we just want the average out of the same EEG file and not another, it will take less time
    [asdf,asdf_hdr] = dual_load_sef(OldFile);
    if size(asdf,1)~=size(EEG,1)
        warning('Number of channels differ between the two files, trying to account for non-ECG & non-EEG channels...')
        
        NoneChannel = find(~cellfun(@isempty,regexpi(Hdr.channelnames','none')));
        MKRchannels = find(~cellfun(@isempty,regexpi(Hdr.channelnames','mkr')));
        ECGchannel = find(~cellfun(@isempty,regexpi(Hdr.channelnames','ecg')));
        
        NewChans = 1:size(EEG,1);
        NewChans([NoneChannel,MKRchannels]) = [];
        NewChans = Hdr.channelnames(NewChans);
        
        if length(NewChans)~=size(asdf,1)
            warning('Could not resolve number of channels mismatch, possibly incorrect channel label for channel highlighted in blue in next figure !')
        end
        [~,~,labelsB,~,asdfb] = bipolar_montage(NewChans,2,asdf);
    else
        [~,~,~,~,asdfb] = bipolar_montage(Hdr.channelnames,1,asdf);
    end
else
...
h = plot_spike( AvgSpike', find(ChannelToShow), asdf_hdr.samplingfreq, labelsB' );
